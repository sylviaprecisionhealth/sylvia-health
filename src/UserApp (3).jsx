    ...
    <div style={{minHeight:'100vh',background:'#080A16',fontFamily:"'DM Sans',sans-serif",paddingBottom:90}}>
      ...
      <div style={{maxWidth:480,margin:'0 auto',padding:'0 20px'}}>
        ...
        {tab==='home'&&<>
          ...
          <div style={{background:'#0d1120',border:'1px solid #1e2640',borderRadius:12,padding:12,marginBottom:12,fontSize:11,color:'#6B6888',lineHeight:1.8}}>
            <div>User ID: <span style={{color:'#A89FFF'}}>{user.id}</span></div>
            <div>All schedules loaded: <span style={{color:'#A89FFF'}}>{schedules.length}</span></div>
            <div>My schedules: <span style={{color:'#A89FFF'}}>{mySchedules.length}</span></div>
            <div>Questions loaded: <span style={{color:'#A89FFF'}}>{questions.length}</span></div>
            <div>Schedule expired: <span style={{color:'#A89FFF'}}>{String(scheduleExpired)}</span></div>
            {schedules.slice(0,2).map((s,i)=><div key={i}>Sched {i}: userId=<span style={{color:'#F6C549'}}>{s.userId}</span> active=<span style={{color:'#F6C549'}}>{String(s.active)}</span></div>)}
          </div>
